...
const AuctionManagerStateV2Struct = struct<AuctionManagerStateV2>(
  [
    ["status", "u8"],
...
const AuctionManagerV2Struct = struct<AuctionManagerV2Data>(
  [
    ["key", "u8"],
...
export class AuctionManager extends MetaplexProgram<AuctionManagerV2Data> {
  ...
  static isAuctionManager(data: Buffer) {
    return (
      AuctionManager.isAuctionManagerV1(data) ||
      AuctionManager.isAuctionManagerV2(data)
    );
  }
